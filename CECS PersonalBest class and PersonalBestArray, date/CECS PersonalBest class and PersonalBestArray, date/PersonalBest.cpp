...
PersonalBest::PersonalBest(MyString  aName, int aMonth, int aDay, int aYear, int aFeet, int aInches, int aJumps) {
    aName = name;
    date = nullptr;
    feetInches = nullptr;
}
...
istream& operator >> (istream& strm, PersonalBest& obj) {
    ...
    cout << "---------------\n";
    obj.orderJumps();
    //highest = obj.feetInches[2].getMeters();
    return strm;
}
