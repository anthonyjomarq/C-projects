...
public:
	...
	virtual ~SimpleVector(); // Destructor declaration

	int getArraySize() const; // Accessor to return the array size
	int getItemCount() const; // Accessor to return a specific element
	T getElementAt(int position); // Overloaded [] operator declaration
...
SimpleVector<T>::SimpleVector(const SimpleVector& obj) {
	...
	arraySize = obj.arraySize; // Copy the array size.
	itemCount = obj.itemCount; // Copy the array size.

	aptr = new T[arraySize]; // Allocate memory for the array.
	if (aptr == 0) {
		memError();
	}

	for (int count = 0; count < getItemCount(); count++) // Copy the elements of obj's array. 
	{
...
void SimpleVector<T>::display() const {
	//cout << "=====================" << endl;
	cout << "The contents in the array are: \n";
